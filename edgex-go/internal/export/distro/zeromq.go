...
func ZeroMQReceiver(eventCh chan *models.Event) {
	fmt.Println("ZeroMQ receiver started")
	go initZmq(eventCh)
}
...
func initZmq(eventCh chan *models.Event) {
	...
	defer q.Close()
	fmt.Println("New Connecting to incoming 0MQ at: ")
	fmt.Println("New Connecting to incoming 0MQ at: " + Configuration.MessageQueue.Uri())

	LoggingClient.Info("New Connecting to incoming 0MQ at: " + Configuration.MessageQueue.Uri())
