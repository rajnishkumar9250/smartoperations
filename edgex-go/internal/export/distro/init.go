...
func Retry(useConsul bool, useProfile string, timeout int, wait *sync.WaitGroup, ch chan error) {
	fmt.Println("Retry fn running is %X", useConsul)
...
func Init(useConsul bool) bool {
	fmt.Println("Init fn called in event")
	if Configuration == nil {
		return false
	}
	if useConsul {
...
func connectToConsul(conf *ConfigurationStruct) (*ConfigurationStruct, error) {
	//Obtain ConsulConfig
	fmt.Println("connectToConsul running")
	fmt.Println(conf.Registry)
	fmt.Println("connectToConsul Registry config is ")
	...
	dec.Prefix = internal.ConfigV2Stem + internal.ExportDistroServiceKey
	dec.ErrCh = errCh
...
func initializeConfiguration(useConsul bool, useProfile string) (*ConfigurationStruct, error) {
	fmt.Println("initializeConfiguration running")
...
func listenForConfigChanges() {
	errCh := make(chan error)
	fmt.Println("listenForConfigChanges")
