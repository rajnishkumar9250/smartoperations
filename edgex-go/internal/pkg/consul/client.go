...
func NewConsulDecoder(reg config.RegistryInfo) *consulstructure.Decoder {
	fmt.Println("NewConsultDecoder fn called")
...
func ConsulInit(config ConsulConfig) error {
	fmt.Println("ConsulInit fn called")
	...
	fmt.Println("defaultConfig.Address is %X", defaultConfig.Address)
